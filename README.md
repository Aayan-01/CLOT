...
1. **Environment Variables**
  ...
  Note about committing built assets

  - This repository now includes a committed `client/dist/` build output to make it straightforward to host a static bundle directly from the repo or to have an initial artifact available. This directory is generated by running `cd client && npm run build` and normally would be ignored, but we've added an exception so it can be checked in when you want a snapshot of production assets.
  - Warning: Committed build artifacts are generated files — if you modify the client source, rebuild locally and commit the new dist/ to keep the checked-in build in sync.
